...
export const filterUsersByUrlParams = (params: IRequestQueryUser) => {
  ...
  const filterLastSeen = {
    ...(seen_start &&
      seen_end === undefined && { lastSeen: { $gte: new Date(seen_start) } }),
    ...(seen_end &&
      seen_start === undefined && { lastSeen: { $lte: new Date(seen_end) } }),
    ...(seen_start &&
      seen_end && {
        $and: [
          { lastSeen: { $gte: new Date(seen_start) } },
          { lastSeen: { $lte: new Date(seen_end) } },
        ],
      }),
  };
